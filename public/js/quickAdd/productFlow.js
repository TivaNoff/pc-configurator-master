  ...
  grid.innerHTML = pageItems
    .map((p) => {
      ...
      const title =
        ...
        [p.specs?.manufacturer, p.specs?.series, p.specs?.model].filter(Boolean).join(" ") || // Добавлены ?. для безопасности
        p.opendb_id;
      ...
      const specsLi = getKeySpecs(s, currentCategory) 
        .map((e) => {
            // e.k уже должен быть переведенной строкой из getKeySpecs
            // e.v - это значение характеристики
            return `<li><strong>${e.k}:</strong> ${e.v}</li>`; 
        })
        .join("");
      const addToBuildText = getTranslation('add_to_build_btn_text') || "Add to build";
...
function renderPagination(totalItems) {
  ...
  if (!pageInfoSpan) {
      paginationCtrls.innerHTML = `
        <button id="firstPage" title="${getTranslation('pagination_first_page') || 'First Page'}">&laquo;</button>
        <button id="prevPage" title="${getTranslation('pagination_prev_page') || 'Previous Page'}">&lsaquo;</button>
        <span class="page-info-span"></span>
        <button id="nextPage" title="${getTranslation('pagination_next_page') || 'Next Page'}">&rsaquo;</button>
        <button id="lastPage" title="${getTranslation('pagination_last_page') || 'Last Page'}">&raquo;</button>
      `;
      
      paginationCtrls.querySelector("#firstPage")?.addEventListener('click', () => {
        setCurrentPage(1);
        rerenderCurrentPage();
      });
      paginationCtrls.querySelector("#prevPage")?.addEventListener('click', () => {
        if (currentPage > 1) {
          setCurrentPage(currentPage - 1);
          rerenderCurrentPage();
        }
      });
      paginationCtrls.querySelector("#nextPage")?.addEventListener('click', () => {
        const currentTotalPages = Math.ceil(getFilteredProducts().length / getPageSize()); 
        if (currentPage < currentTotalPages) {
          setCurrentPage(currentPage + 1);
          rerenderCurrentPage();
        }
      });
      paginationCtrls.querySelector("#lastPage")?.addEventListener('click', () => {
        const currentTotalPages = Math.ceil(getFilteredProducts().length / getPageSize()); 
  ...
  if (pageInfoSpan) {
      pageInfoSpan.textContent = getTranslation('pagination_page_info', undefined, { currentPage, totalPages });
  }
  ...
  if (nextBtn) nextBtn.disabled = currentPage === currentTotalPagesForButtons || currentTotalPagesForButtons === 0;
  if (lastBtn) lastBtn.disabled = currentPage === currentTotalPagesForButtons || currentTotalPagesForButtons === 0;
}
